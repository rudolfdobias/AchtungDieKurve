...
{
    public class SlimEnemy : Powerup
    {
        ...
        public SlimEnemy(GameBase game, SpriteBatch spriteBatch, GameplayScreen gameplay)
            : base(game, spriteBatch, gameplay)
        {
        }

        public SlimEnemy(GameBase game, SpriteBatch spriteBatch, GameplayScreen gameplay, Texture2D texture)
        ...
        public override void Apply(Kurve entity, GameTime gameTime)
        {
            for (var i = 0; i < Gameplay.PlayersManager.Worms.Count; i++)
            {
                ...
                p.TurnStep *= AngleFactor;
                if (delta < p.Diameter)
                {
                    _reverse[i] = Thickness - (p.Diameter - delta);
                }
                else
                {
