...
- Display modes are owned by `AchtungDieKurve/Graphics/GraphicsManager.cs`: borderless fullscreen at desktop resolution (default) or windowed at a selectable size. `HardwareModeSwitch` must stay `false` — exclusive mode switches break on macOS (resize flicker, nothing renders). Gameplay tuning (`DefaultDiameter`, `DefaultSpeed`) is scaled there relative to a 1080p baseline whenever the resolution changes. Headless/CI runs are not viable.
...
- The curve trail grows as a side effect of `Kurve.Draw()` (segments are appended to `Body` during rendering), not in `Update()`.
